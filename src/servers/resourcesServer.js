...
const redis = require('../config/redis'); // Ensure this path is correct

const router = express.Router();

router.get('', async (req, res) => {
  try {
    const resources = await redis.getAllResources(); // Calling the correct method
    res.json(resources);
  } catch (error) {
...
router.post('', async (req, res) => {
  try {
    ...
    await redis.createResource(id, newResource); // Ensure createResource exists
